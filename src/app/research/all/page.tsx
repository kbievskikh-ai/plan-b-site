...
export default function AllResearchPage() {
      ...
      .then(data => {
        if (data?.data?.length) {
          const filtered = data.data
            .filter((r: Record<string, unknown>) => String((r as any).status || 'published') !== 'draft')
  ...
  const filtered = filter === 'all' ? items : items.filter(i => SECTIONS.find(s => s.key === filter)?.categories.includes(i.category));
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h1 className="font-heading text-2xl sm:text-3xl lg:text-4xl text-white leading-tight">
              {language === 'ru' ? 'Библиотека исследований' : language === 'pt' ? 'Biblioteca de Pesquisas' : 'Research Library'}
            </h1>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-10">
            ...
            {SECTIONS.map(section => {
              const count = items.filter(i => section.categories.includes(i.category)).length;
              return (
                <Link
                  key={section.key}
                  href={`/research/${section.key}`}
                  className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wider uppercase transition-all duration-300 border ${
                    filter === section.key
                      ? 'bg-gold-500 text-navy-900 shadow-lg shadow-gold-500/20 border-gold-500'
